...
public class Matrix {
    ...
    public static void multiplyMM(float[] d, int dP,
        ...
        for (int i = 0; i < 4; i++) {
            ...
            d[dP+i+1*4] = ai0 * b[bP+0+1*4] + ai1 * b[bP+1+1*4] + ai2 * b[bP+2+1*4] + ai3 * b[bP+3+1*4];
            d[dP+i+2*4] = ai0 * b[bP+0+2*4] + ai1 * b[bP+1+2*4] + ai2 * b[bP+2+2*4] + ai3 * b[bP+3+2*4];
            d[dP+i+3*4] = ai0 * b[bP+0+3*4] + ai1 * b[bP+1+3*4] + ai2 * b[bP+2+3*4] + ai3 * b[bP+3+3*4];
        }
    }
     ...
     */
    public static final void multiplyMV(float[] v_out, int v_out_off, float[] m_in, int m_in_off, float[] v_in, int v_in_off) {
        for (int i = 0; i < 4; i++) {
